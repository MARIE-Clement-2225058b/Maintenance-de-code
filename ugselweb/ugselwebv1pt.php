...
Function export($listetable = "", $auto = "", $externe = "") {
	...
	if (($optionexporttype == "expsql") || ($optionexporttype == "expser") || ($optionexporttype == "expsqlser")) {
		if ($optionexport == "exptout"     ) {$typeexp = "Tout"; $listetable = Array("Param�tres", "Sports", "Etablissements", "Secteurs", "Licenci�s", "Cat�gories", "Comp�titions", "Epreuves", "Epreuves Comp�titions", "Groupes", "Participations", "Equipes", "Tours Epreuves Comp�titions");} 
		if ($optionexport == "expetab"     ) {
			...
			$listetable = array(
				array("Param�tres","SELECT * FROM Param�tres"),
				...
				array("Licenci�s","SELECT Licenci�s.* FROM Licenci�s INNER JOIN Etablissements ON Licenci�s.LicEtabCode = Etablissements.EtabCode $mastrwhere ORDER BY LicNumLicence")
		);}
		if ($optionexport == "explic") {
			$typeexp = "Licenci�s";
			$listetable = array(
				array("Param�tres","SELECT * FROM Param�tres"),
				array("Secteurs","SELECT DISTINCTROW Secteurs.* FROM (Etablissements INNER JOIN Licenci�s ON Etablissements.EtabCode = Licenci�s.LicEtabCode) INNER JOIN Secteurs ON Etablissements.EtabSecCode = Secteurs.SecCode Order By Secteurs.Ordre"),
				array("Etablissements","SELECT DISTINCTROW Etablissements.* FROM Licenci�s INNER JOIN Etablissements ON Licenci�s.LicEtabCode = Etablissements.EtabCode ORDER BY EtabNum"),
				array("Licenci�s","SELECT * FROM Licenci�s ORDER BY LicNumLicence")
		);} 
		if ($optionexport == "expsport"    ) {
			...
			$listetable = array(
				...
				array("Cat�gories","SELECT * FROM Cat�gories WHERE Cat�gories.CatSpoCode = $ListeExportSport ORDER BY Ordre"),
				array("Epreuves","SELECT Epreuves.* FROM Cat�gories INNER JOIN Epreuves ON Cat�gories.CatCode = Epreuves.EprCatCode WHERE EprSpoCode = $ListeExportSport ORDER BY Epreuves.Ordre"),
		...
		if ($optionexport == "expcompet"   ) {
			...
			$listetable = array(
				array("Param�tres","SELECT * FROM Param�tres"),
				array("Sports","SELECT Sports.* FROM Sports INNER JOIN Comp�titions ON Sports.SpoCode = Comp�titions.CompetSpoCode WHERE Comp�titions.CompetCode = $ListeExportCompet"),
				array("Etablissements","(SELECT DISTINCTROW Etablissements.* FROM Cat�gories RIGHT JOIN ((Epreuves RIGHT JOIN `Epreuves Comp�titions` ON Epreuves.EprCode = `Epreuves Comp�titions`.EprCompetEprCode) RIGHT JOIN (Equipes INNER JOIN Etablissements ON Equipes.EquEtabCode = Etablissements.EtabCode) ON `Epreuves Comp�titions`.EprCompetCode = Equipes.EquEprCompetCode) ON Cat�gories.CatCode = Epreuves.EprCatCode WHERE Equipes.EquCompetCode = $ListeExportCompet) 
							UNION (SELECT DISTINCTROW Etablissements.* FROM Cat�gories INNER JOIN ((Etablissements INNER JOIN Licenci�s ON Etablissements.EtabCode = Licenci�s.LicEtabCode) INNER JOIN (Epreuves INNER JOIN (`Epreuves Comp�titions` INNER JOIN Participations ON `Epreuves Comp�titions`.EprCompetCode = Participations.ParEprCode) ON Epreuves.EprCode = `Epreuves Comp�titions`.EprCompetEprCode) ON Licenci�s.LicNumLicence = Participations.ParLicCode) ON Cat�gories.CatCode = Epreuves.EprCatCode WHERE Participations.ParCompetCode = $ListeExportCompet ORDER BY EtabNum)
							UNION (SELECT DISTINCTROW Etablissements.* FROM Etablissements INNER JOIN Licenci�s ON Etablissements.EtabCode = Licenci�s.LicEtabCode WHERE LicNumLicence IN ($strRel)) "),
				array("Secteurs","(SELECT DISTINCTROW Secteurs.* FROM (Cat�gories INNER JOIN ((Etablissements INNER JOIN Licenci�s ON Etablissements.EtabCode = Licenci�s.LicEtabCode) INNER JOIN (Epreuves INNER JOIN (`Epreuves Comp�titions` INNER JOIN Participations ON `Epreuves Comp�titions`.EprCompetCode = Participations.ParEprCode) ON Epreuves.EprCode = `Epreuves Comp�titions`.EprCompetEprCode) ON Licenci�s.LicNumLicence = Participations.ParLicCode) ON Cat�gories.CatCode = Epreuves.EprCatCode) INNER JOIN Secteurs ON Etablissements.EtabSecCode = Secteurs.SecCode WHERE Participations.ParCompetCode = $ListeExportCompet)
							UNION (SELECT DISTINCTROW Secteurs.* FROM (Cat�gories RIGHT JOIN ((Epreuves RIGHT JOIN `Epreuves Comp�titions` ON Epreuves.EprCode = `Epreuves Comp�titions`.EprCompetEprCode) RIGHT JOIN (Equipes INNER JOIN Etablissements ON Equipes.EquEtabCode = Etablissements.EtabCode) ON `Epreuves Comp�titions`.EprCompetCode = Equipes.EquEprCompetCode) ON Cat�gories.CatCode = Epreuves.EprCatCode) INNER JOIN Secteurs ON Etablissements.EtabSecCode = Secteurs.SecCode WHERE Equipes.EquCompetCode = $ListeExportCompet ORDER BY Ordre)
							UNION (SELECT DISTINCTROW Secteurs.* FROM Secteurs INNER JOIN Etablissements ON Secteurs.SecCode = Etablissements.EtabSecCode INNER JOIN Licenci�s ON Etablissements.EtabCode = Licenci�s.LicEtabCode WHERE LicNumLicence IN ($strRel)) "),
				array("Licenci�s","(SELECT DISTINCTROW Licenci�s.* FROM (Cat�gories INNER JOIN Epreuves ON Cat�gories.CatCode = Epreuves.EprCatCode) INNER JOIN (`Epreuves Comp�titions` INNER JOIN (Licenci�s INNER JOIN Participations ON Licenci�s.LicNumLicence = Participations.ParLicCode) ON `Epreuves Comp�titions`.EprCompetCode = Participations.ParEprCode) ON Epreuves.EprCode = `Epreuves Comp�titions`.EprCompetEprCode WHERE Participations.ParCompetCode = $ListeExportCompet ORDER BY Licenci�s.LicNumLicence)
							UNION (SELECT DISTINCTROW Licenci�s.* FROM Licenci�s WHERE LicNumLicence IN ($strRel) )"),
				array("Cat�gories","SELECT Cat�gories.* FROM (Sports INNER JOIN Cat�gories ON Sports.SpoCode = Cat�gories.CatSpoCode) INNER JOIN Comp�titions ON Sports.SpoCode = Comp�titions.CompetSpoCode WHERE CompetCode = $ListeExportCompet ORDER BY Cat�gories.Ordre"),
				array("Comp�titions","SELECT * FROM Comp�titions WHERE CompetCode = $ListeExportCompet"),
				array("Epreuves","SELECT Epreuves.* FROM Epreuves INNER JOIN `Epreuves Comp�titions` ON Epreuves.EprCode = `Epreuves Comp�titions`.EprCompetEprCode WHERE `Epreuves Comp�titions`.EprCompetCompetCode = $ListeExportCompet ORDER BY Epreuves.Ordre"),
				array("Epreuves Comp�titions","SELECT * FROM `Epreuves Comp�titions` WHERE EprCompetCompetCode = $ListeExportCompet"),
				array("Participations","SELECT Participations.* FROM (Cat�gories INNER JOIN Epreuves ON Cat�gories.CatCode = Epreuves.EprCatCode) INNER JOIN (`Epreuves Comp�titions` INNER JOIN Participations ON `Epreuves Comp�titions`.EprCompetCode = Participations.ParEprCode) ON Epreuves.EprCode = `Epreuves Comp�titions`.EprCompetEprCode WHERE Participations.ParCompetCode = $ListeExportCompet ORDER BY Participations.ParLicCode, Participations.ParTour, Epreuves.Ordre"),
				array("Equipes","SELECT Equipes.* FROM Cat�gories RIGHT JOIN ((Epreuves RIGHT JOIN `Epreuves Comp�titions` ON Epreuves.EprCode = `Epreuves Comp�titions`.EprCompetEprCode) RIGHT JOIN Equipes ON `Epreuves Comp�titions`.EprCompetCode = Equipes.EquEprCompetCode) ON Cat�gories.CatCode = Epreuves.EprCatCode WHERE Equipes.EquCompetCode = $ListeExportCompet ORDER BY Cat�gories.Ordre, Epreuves.Ordre, Equipes.EquNum, Equipes.EquTour"),
				array("Groupes","SELECT Groupes.* FROM (Sports INNER JOIN Groupes ON Sports.SpoCode = Groupes.GrSpoCode) INNER JOIN Comp�titions ON Sports.SpoCode = Comp�titions.CompetSpoCode WHERE Comp�titions.CompetCode = $ListeExportCompet ORDER BY Groupes.Ordre"),
				array("Tours Epreuves Comp�titions","SELECT `Tours Epreuves Comp�titions`.* FROM `Epreuves Comp�titions` INNER JOIN `Tours Epreuves Comp�titions` ON `Epreuves Comp�titions`.EprCompetCode = `Tours Epreuves Comp�titions`.TouEprCompetEprCompetCode WHERE EprCompetCompetCode = $ListeExportCompet")
			);
			
			$req = bf_mysql_query("SELECT Sports.*, Comp�titions.* FROM Sports INNER JOIN Comp�titions ON Sports.SpoCode = Comp�titions.CompetSpoCode WHERE Comp�titions.CompetCode = $ListeExportCompet");
			if (!(!$req)) {
				$res = mysql_fetch_array($req);
				if (!(!$res)) {
					$MonSport = $res["SpoLibelCourt"];
					$MaDescription = $res["CompetLibell�"]." ".eregi_replace("([0-9].*)-([0-9].*)-([0-9].*)" ,"\\3/\\2/\\1",$res["CompetDateD�b"])." ".$res["CompetLieu"];
		...
		if ($optionexport == "expetab"     ) {
			...
			$listetable = array(
				array("Etablissements","SELECT EtabNum As Num�ro, EtabNom As Nom, EtabAdresse1 As Adresse1, EtabAdresse2 As Adresse2, EtabCp As CP, EtabVille As Ville, EtabT�l As T�l, EtabFax As Fax, EtabMemo3 AS Mdp FROM Etablissements $mastrwhere ORDER BY EtabNum"),
				array("Licenci�s","SELECT IF(Licenci�s.LicInscrit=1, 'X','') As Inscrit, Licenci�s.LicNumLicence As Num�ro, Licenci�s.LicNom As Nom, Licenci�s.LicPr�nom As Pr�nom, Licenci�s.LicNaissance As Naiss, IF(Licenci�s.LicSexCode=1,'G','F') As Sexe , CatLibelCourt As Cat, EtabNum, EtabNomCourt As EtabCode, EtabNom , EtabVille FROM Cat�gories, Etablissements INNER JOIN Licenci�s ON Etablissements.EtabCode = Licenci�s.LicEtabCode WHERE ((Licenci�s.LicNaissance Between CatDateD�b And CatDateFin) And LicSexCode = CatSexCode And CatSpoCode = 1) $mastrwhere1 ORDER BY Licenci�s.LicNumLicence"),
		...
		if ($optionexport == "explic") {
			...
			$listetable = array(
				array("Licenci�s","SELECT IF(Licenci�s.LicInscrit=1, 'X','') As Inscrit, Licenci�s.LicNumLicence As Num�ro, Licenci�s.LicNom As Nom, Licenci�s.LicPr�nom As Pr�nom, Licenci�s.LicNaissance As Naiss, IF(Licenci�s.LicSexCode=1,'G','F') As Sexe , CatLibelCourt As Cat $mastrchpetab FROM Cat�gories, Etablissements INNER JOIN Licenci�s ON Etablissements.EtabCode = Licenci�s.LicEtabCode WHERE (Licenci�s.LicNaissance Between CatDateD�b And CatDateFin) And LicSexCode = CatSexCode And CatSpoCode = 1 $mastrwhere ORDER BY Licenci�s.LicNumLicence"),
		...
		if ($optionexport == "expsport") {
			$typeexp = "Sports";
			$listetable = array(
				array("Sports","SELECT SpoLibelCourt As Sport, SpoLibell� As Lib�ll� FROM Sports WHERE SpoCode = $ListeExportSport"),
				array("Cat�gories","SELECT SpoLibelCourt As Sport, CatLibelCourt As Cat�gorie, CatLibell� As Libell�, CatDateD�b As D�but, CatDateFin As Fin, IF(CatSexCode = 1,'G','F') As Sexe FROM `Cat�gories` INNER JOIN `Sports` ON Cat�gories.CatSpoCode = Sports.SpoCode WHERE Cat�gories.CatSpoCode = $ListeExportSport ORDER BY Cat�gories.Ordre"),
				array("Epreuves","SELECT SpoLibelCourt As Sport, CatLibelCourt As Cat�gorie, EprLibelCourt As Epreuve, EprLibell� As Libell�, IF(CatSexCode = 1,'G','F') As Sexe FROM (Sports INNER JOIN Epreuves ON Sports.SpoCode = Epreuves.EprSpoCode) INNER JOIN Cat�gories ON Epreuves.EprCatCode = Cat�gories.CatCode WHERE EprSpoCode = $ListeExportSport ORDER BY Epreuves.Ordre"),
		);} 
		if ($optionexport == "expcompet") {
			$typeexp = "Comp�tition";
			$req = bf_mysql_query("SELECT Comp�titions.* FROM Comp�titions WHERE CompetCode = $ListeExportCompet");
			if (!(!$req)) {
				$res = mysql_fetch_array($req);
			...
			if ($ADMINREGLOGIN == "bzh") {				
				$listetable = array(
					array("Comp�titions","SELECT CompetLibell� As Comp�tition, CompetDateD�b As Date, CompetLieu As Lieu, SpoLibelCourt As Sport, CompetObs As Obs FROM Sports INNER JOIN Comp�titions ON Sports.SpoCode = Comp�titions.CompetSpoCode WHERE CompetCode = $ListeExportCompet"),
					array("Etablissements","(SELECT DISTINCTROW EtabNum As Num�ro, EtabNomCourt As Etab, EtabNom As Nom, EtabVille AS Ville, EtabAdresse1 As Adresse1, EtabAdresse2 As Adresse2, EtabCp As CP, EtabVille As Ville, EtabT�l As T�l, EtabFax As Fax FROM Cat�gories RIGHT JOIN ((Epreuves RIGHT JOIN `Epreuves Comp�titions` ON Epreuves.EprCode = `Epreuves Comp�titions`.EprCompetEprCode) RIGHT JOIN (Equipes INNER JOIN Etablissements ON Equipes.EquEtabCode = Etablissements.EtabCode) ON `Epreuves Comp�titions`.EprCompetCode = Equipes.EquEprCompetCode) ON Cat�gories.CatCode = Epreuves.EprCatCode WHERE Equipes.EquCompetCode = $ListeExportCompet $mastrwhere) UNION (SELECT DISTINCTROW EtabNum As Num�ro, EtabNomCourt AS Etab, EtabNom As Nom, EtabVille AS Ville, EtabAdresse1 As Adresse1, EtabAdresse2 As Adresse2, EtabCp As CP, EtabVille As Ville, EtabT�l As T�l, EtabFax As Fax FROM Cat�gories INNER JOIN ((Etablissements INNER JOIN Licenci�s ON Etablissements.EtabCode = Licenci�s.LicEtabCode) INNER JOIN (Epreuves INNER JOIN (`Epreuves Comp�titions` INNER JOIN Participations ON `Epreuves Comp�titions`.EprCompetCode = Participations.ParEprCode) ON Epreuves.EprCode = `Epreuves Comp�titions`.EprCompetEprCode) ON Licenci�s.LicNumLicence = Participations.ParLicCode) ON Cat�gories.CatCode = Epreuves.EprCatCode WHERE Participations.ParCompetCode = $ListeExportCompet $mastrwhere) ORDER BY Num�ro"),
					array("Participations","SELECT ParLicCode As Num�ro, Licenci�s.LicNom As Nom, Licenci�s.LicPr�nom As Pr�nom, Licenci�s.LicNaissance As Naiss, IF(CatSexCode = 1,'G','F') As Sexe, CatLibelCourt As Cat, EtabNomCourt As Etab, EtabNom As `Etab Nom`, EtabVille AS `Etab Ville`, EprLibelCourt As Epr, EquNum As Equ, ParObs1 As Obs1, ParObs2 As Obs2, ParObs3 As Obs3, ParObs4 As Obs4, ParObs5 As Obs5 FROM Cat�gories, Etablissements INNER JOIN Licenci�s ON Etablissements.EtabCode = Licenci�s.LicEtabCode INNER JOIN Participations ON Licenci�s.LicNumLicence = Participations.ParLicCode INNER JOIN `Epreuves Comp�titions` ON `Epreuves Comp�titions`.EprCompetCode = Participations.ParEprCode INNER JOIN Epreuves ON Epreuves.EprCode = `Epreuves Comp�titions`.EprCompetEprCode LEFT JOIN Equipes ON Equipes.EquCode = Participations.ParEquCode WHERE ((Licenci�s.LicNaissance Between CatDateD�b And CatDateFin) And LicSexCode = CatSexCode And CatSpoCode = 1) AND ParCompetCode = $ListeExportCompet $mastrwhere ORDER BY LicNom, LicPr�nom, Participations.ParLicCode, Participations.ParTour, Epreuves.Ordre"),
					array("",""),
					array("",""),
					array("Participations","SELECT EtabNomCourt As Etab, EtabNom As `Etab Nom`, EtabVille AS `Etab Ville`, ParLicCode As Num�ro, Licenci�s.LicNom As Nom, Licenci�s.LicPr�nom As Pr�nom, Licenci�s.LicNaissance As Naiss, IF(CatSexCode = 1,'G','F') As Sexe, CatLibelCourt As Cat, EprLibelCourt As Epr, EquNum As Equ, ParObs1 As Obs1, ParObs2 As Obs2, ParObs3 As Obs3, ParObs4 As Obs4, ParObs5 As Obs5 FROM Cat�gories, Etablissements INNER JOIN Licenci�s ON Etablissements.EtabCode = Licenci�s.LicEtabCode INNER JOIN Participations ON Licenci�s.LicNumLicence = Participations.ParLicCode INNER JOIN `Epreuves Comp�titions` ON `Epreuves Comp�titions`.EprCompetCode = Participations.ParEprCode INNER JOIN Epreuves ON Epreuves.EprCode = `Epreuves Comp�titions`.EprCompetEprCode LEFT JOIN Equipes ON Equipes.EquCode = Participations.ParEquCode WHERE ((Licenci�s.LicNaissance Between CatDateD�b And CatDateFin) And LicSexCode = CatSexCode And CatSpoCode = 1) AND ParCompetCode = $ListeExportCompet $mastrwhere ORDER BY EtabNomCourt, LicEtabCode, EquNum, LicNom, LicPr�nom, Participations.ParLicCode, Participations.ParTour, Epreuves.Ordre"),
					array("Equipes","SELECT EquNum As Num�ro, EquCompl�ment As Compl�ment, EtabNomCourt As Etab, EtabNom As EtabNom, EtabVille As EtabVille, CatLibelCourt As Cat, EprLibelCourt As Epreuve, IF (EquPromo = 1, 'X','') As Promo, EquChall As Chall, EquRelayeurs As Relayeurs, EquObs1 As Obs1, EquObs2 As Obs2, EquObs3 As Obs3, EquObs4 As Obs4, EquObs5 As Obs5 FROM Etablissements, Cat�gories INNER JOIN Equipes ON Cat�gories.CatCode = Equipes.EquCatCode LEFT JOIN `Epreuves Comp�titions` ON Equipes.EquEprCompetCode = `Epreuves Comp�titions`.EprCompetCode LEFT JOIN Epreuves ON `Epreuves Comp�titions`.EprCompetEprCode = Epreuves.EprCode WHERE Etablissements.EtabCode = Equipes.EquEtabCode AND EquCompetCode = $ListeExportCompet $mastrwhere $mastrwhere ORDER BY EtabNomCourt, EquNum, EquCompl�ment, Epreuves.Ordre"),
				...
				$listetable = array(
					array("Comp�titions","SELECT CompetLibell� As Comp�tition, CompetDateD�b As Date, CompetLieu As Lieu, SpoLibelCourt As Sport, CompetObs As Obs FROM Sports INNER JOIN Comp�titions ON Sports.SpoCode = Comp�titions.CompetSpoCode WHERE CompetCode = $ListeExportCompet"),
					array("Etablissements","(SELECT DISTINCTROW EtabNum As Num�ro, EtabNomCourt As Etab, EtabNom As Nom, EtabVille AS Ville, EtabAdresse1 As Adresse1, EtabAdresse2 As Adresse2, EtabCp As CP, EtabVille As Ville, EtabT�l As T�l, EtabFax As Fax FROM Cat�gories RIGHT JOIN ((Epreuves RIGHT JOIN `Epreuves Comp�titions` ON Epreuves.EprCode = `Epreuves Comp�titions`.EprCompetEprCode) RIGHT JOIN (Equipes INNER JOIN Etablissements ON Equipes.EquEtabCode = Etablissements.EtabCode) ON `Epreuves Comp�titions`.EprCompetCode = Equipes.EquEprCompetCode) ON Cat�gories.CatCode = Epreuves.EprCatCode WHERE Equipes.EquCompetCode = $ListeExportCompet $mastrwhere) UNION (SELECT DISTINCTROW EtabNum As Num�ro, EtabNomCourt AS Etab, EtabNom As Nom, EtabVille AS Ville, EtabAdresse1 As Adresse1, EtabAdresse2 As Adresse2, EtabCp As CP, EtabVille As Ville, EtabT�l As T�l, EtabFax As Fax FROM Cat�gories INNER JOIN ((Etablissements INNER JOIN Licenci�s ON Etablissements.EtabCode = Licenci�s.LicEtabCode) INNER JOIN (Epreuves INNER JOIN (`Epreuves Comp�titions` INNER JOIN Participations ON `Epreuves Comp�titions`.EprCompetCode = Participations.ParEprCode) ON Epreuves.EprCode = `Epreuves Comp�titions`.EprCompetEprCode) ON Licenci�s.LicNumLicence = Participations.ParLicCode) ON Cat�gories.CatCode = Epreuves.EprCatCode WHERE Participations.ParCompetCode = $ListeExportCompet $mastrwhere) ORDER BY Num�ro"),
					array("Participations","SELECT ParLicCode As Num�ro, EtabNomCourt As Etab, EtabNom As `Etab Nom`, EtabVille AS `Etab Ville`, Licenci�s.LicNom As Nom, Licenci�s.LicPr�nom As Pr�nom, Licenci�s.LicNaissance As Naiss, IF(CatSexCode = 1,'G','F') As Sexe, CatLibelCourt As Cat, EprLibelCourt As Epr, EquNum As Equ, ParObs1 As Obs1, ParObs2 As Obs2, ParObs3 As Obs3, ParObs4 As Obs4, ParObs5 As Obs5 FROM Cat�gories, Etablissements INNER JOIN Licenci�s ON Etablissements.EtabCode = Licenci�s.LicEtabCode INNER JOIN Participations ON Licenci�s.LicNumLicence = Participations.ParLicCode INNER JOIN `Epreuves Comp�titions` ON `Epreuves Comp�titions`.EprCompetCode = Participations.ParEprCode INNER JOIN Epreuves ON Epreuves.EprCode = `Epreuves Comp�titions`.EprCompetEprCode LEFT JOIN Equipes ON Equipes.EquCode = Participations.ParEquCode WHERE ((Licenci�s.LicNaissance Between CatDateD�b And CatDateFin) And LicSexCode = CatSexCode And CatSpoCode = 1) AND ParCompetCode = $ListeExportCompet $mastrwhere ORDER BY Participations.ParLicCode, Participations.ParTour, Epreuves.Ordre"),
					array("Equipes","SELECT EquNum As Num�ro, EquCompl�ment As Compl�ment, EtabNomCourt As Etab, EtabNom As EtabNom, EtabVille As EtabVille, CatLibelCourt As Cat, EprLibelCourt As Epreuve, IF (EquPromo = 1, 'X','') As Promo, EquChall As Chall, EquRelayeurs As Relayeurs, EquObs1 As Obs1, EquObs2 As Obs2, EquObs3 As Obs3, EquObs4 As Obs4, EquObs5 As Obs5 FROM Etablissements, Cat�gories INNER JOIN Equipes ON Cat�gories.CatCode = Equipes.EquCatCode LEFT JOIN `Epreuves Comp�titions` ON Equipes.EquEprCompetCode = `Epreuves Comp�titions`.EprCompetCode LEFT JOIN Epreuves ON `Epreuves Comp�titions`.EprCompetEprCode = Epreuves.EprCode WHERE Etablissements.EtabCode = Equipes.EquEtabCode AND EquCompetCode = $ListeExportCompet $mastrwhere $mastrwhere ORDER BY EtabNomCourt, EquNum, EquCompl�ment, Epreuves.Ordre"),
					array("",ConstruitStat($compets, $MonSportCode, $queryStr, $NomsColonnes, $ChampsAli, $ChampsFor, $ChampsAff, $ChampsType, $Choix, $mastrwhere))
				);
...
function RetourneFic ($chemin = ".", $Masque = "", $Type = "", $TriFic = "") {
	...
	while ($file = @readdir($handle)) {
		if( (!(is_dir("$chemin/$file"))) && (!(!(strstr($file, $Masque)))) ){		
			...
			$tabfile[$fileind]["Date"]      = array_search(Date("w", filemtime("$chemin/$file")), $arrayjours) . " ugselwebv1pt.php" .Date("d/m/Y", filemtime("$chemin/$file"));
...
function GereData($tablename, $queryStr, $MaKey="", $NomsColonnes="", $ChampsTri="", $ChampsAli="", $ChampsFor="", $ChampsAff="", $action="GereData", $orderby="", $Choix="", $ChampsEdit="", $ChampsInsert="", $ChampsType="", $ChampsTypeExt="", $ChampsFiltre="", $where="", $ChampsNomFil="", $ChampsRacFiltre = "", $ChampsRacParam = "", $sousqueryStr, $messagedel = "", $MajChpOrdre = "", $stat = 0, $strCatEpr = "", $maxInsc = 99999 ) {
			...
			if (($menu == "competitions") && ($sousmenu == "individuels")) {
				if ((!(isset($BValidernumlicence))) || (empty($BValidernumlicence))) {
					...
					if ($reqlic) {
						...
						if ($reslic) {
							$reflic = sprintf('%010s', $reslic['LicNumLicence']) . " ugselwebv1pt.php" .$reslic['LicNom']." ".$reslic['LicPr�nom'];
	...
	if (($menu == "parametres") && ($sousmenu == "sports") && (isset($valideimportsport))) {
		...
		$reqImport = bf_mysql_query("SELECT `".$trans." Sports`.*, `".$trans." Cat�gories`.*, `".$trans." Groupes`.*, `".$trans." Epreuves`.* FROM `".$trans." Sports` INNER JOIN `".$trans." Cat�gories` On `".$trans." Sports`.SpoCode = `".$trans." Cat�gories`.CatSpoCode  INNER JOIN `".$trans." Epreuves` ON `".$trans." Cat�gories`.CatCode = `".$trans." Epreuves`.EprCatCode LEFT JOIN `".$trans." Groupes` ON `".$trans." Groupes`.GrCode = `".$trans." Epreuves`.EprGrCode WHERE SpoCode = ".$ListeImportSport);
		if (!(!($reqImport))) {	
			while ($resImport = mysql_fetch_array($reqImport)) { 
				Maj(1,"Cat�gories", $resImport, Array("CatSpoCode" => "SELECT SpoCode FROM Sports WHERE SpoCode = ".$Sport));
				Maj(1,"Groupes", $resImport, Array("GrSpoCode" => "SELECT SpoCode FROM Sports WHERE SpoCode = ".$Sport));	
				Maj(1,"Epreuves", $resImport, Array("EprSpoCode" => "SELECT SpoCode FROM Sports WHERE SpoCode = ".$Sport, "EprCatCode" => "SELECT CatCode FROM Cat�gories WHERE CatLibelCourt = '".$resImport["CatLibelCourt"]."'", "EprGrCode" => "SELECT GrCode FROM Groupes INNER JOIN Sports ON Groupes.GrSpoCode = Sports.SpoCode WHERE GrLibelCourt = ".$resImport["GrLibelCourt"]. " AND SpoCode = ".$Sport));
			} 
		}
		$reqImport = bf_mysql_query("SELECT `".$trans." Sports`.*, `".$trans." Cat�gories`.* FROM `".$trans." Sports` INNER JOIN `".$trans." Cat�gories` On `".$trans." Sports`.SpoCode = `".$trans." Cat�gories`.CatSpoCode WHERE (`".$trans." Cat�gories`.CatType = 99 OR `".$trans." Cat�gories`.CatType = 1) AND SpoCode = ".$ListeImportSport);
		if (!(!($reqImport))) {	
			while ($resImport = mysql_fetch_array($reqImport)) {
				Maj(1,"Cat�gories", $resImport, Array("CatSpoCode" => "SELECT SpoCode FROM Sports WHERE SpoCode = ".$Sport)); 	
			}
	...
	if (($menu == "competitions") && ($sousmenu == "references") && (isset($valideimportcompet))) {
		...
		$reqImportEqu = bf_mysql_query("SELECT * FROM Comp�titions WHERE CompetEqu = 1 AND CompetCode = $Compet");
		if ((!(!($reqImportEqu))) && (mysql_num_rows($reqImportEqu) > 0)) {
			$reqImport = bf_mysql_query("SELECT `".$trans." Secteurs`. * , `".$trans." Etablissements`. * , `".$trans." Equipes`. * , `".$trans." Epreuves Comp�titions`. * , `".$trans." Epreuves`. * , `".$trans." Comp�titions`. * , `".$trans." Sports`. * , `".$trans." Cat�gories`. * FROM `".$trans." Equipes`INNER JOIN `".$trans." Etablissements` ON `".$trans." Equipes`.EquEtabCode = `".$trans." Etablissements`.EtabCode INNER JOIN `".$trans." Secteurs` ON `".$trans." Etablissements`.EtabSecCode = `".$trans." Secteurs`.SecCode LEFT JOIN `".$trans." Epreuves Comp�titions` ON `".$trans." Epreuves Comp�titions`.EprCompetCode = `".$trans." Equipes`.EquEprCompetCode LEFT JOIN `".$trans." Epreuves` ON `".$trans." Epreuves Comp�titions`.EprCompetEprCode = `".$trans." Epreuves`.EprCode INNER JOIN `".$trans." Comp�titions` ON `".$trans." Comp�titions`.CompetCode = `".$trans." Equipes`.EquCompetCode INNER JOIN `".$trans." Sports` ON `".$trans." Sports`.SpoCode = `".$trans." Comp�titions`.CompetSpoCode LEFT JOIN `".$trans." Cat�gories` ON `".$trans." Equipes`.EquCatCode = `".$trans." Cat�gories`.CatCode WHERE `".$trans." Equipes`.EquCompetCode = ".$ListeImportCompet);
			if (!(!($reqImport))) {
				while ($resImport = mysql_fetch_array($reqImport)) {
					...
					Maj(1,"Equipes", $resImport, Array(
						...
						"EquCatCode"  => "SELECT CatCode FROM Cat�gories INNER JOIN Sports ON Cat�gories.CatSpoCode = Sports.SpoCode INNER JOIN Comp�titions ON Comp�titions.CompetSpoCode = Sports.SpoCode WHERE Cat�gories.CatLibelCourt = '".$resImport["CatLibelCourt"]."' AND CompetCode = $Compet",
						"EquCompetCode" => "SELECT CompetCode From Comp�titions Where CompetCode = ".$Compet, 
						"EquEprCompetCode" => "SELECT EprCompetCode FROM Sports INNER JOIN Epreuves ON Sports.SpoCode = Epreuves.EprSpoCode INNER JOIN `Epreuves Comp�titions` ON `Epreuves Comp�titions`.EprCompetEprCode = Epreuves.EprCode WHERE SpoLibelCourt = '".$resImport["SpoLibelCourt"]."' AND EprLibelCourt = '".$resImport["EprLibelCourt"]."'"
					));
		...
		$reqImport = bf_mysql_query("SELECT DISTINCT `".$trans." Secteurs`.*, `".$trans." Etablissements`.* FROM `".$trans." Licenci�s` INNER JOIN `".$trans." Etablissements` ON `".$trans." Licenci�s`.LicEtabCode = `".$trans." Etablissements`.EtabCode INNER JOIN `".$trans." Secteurs` ON `".$trans." Etablissements`.EtabSecCode = `".$trans." Secteurs`.SecCode INNER JOIN `".$trans." Participations`On `".$trans." Participations`.ParLicCode = `".$trans." Licenci�s`.LicNumLicence INNER JOIN `".$trans." Epreuves Comp�titions` ON `".$trans." Epreuves Comp�titions`.EprCompetCode = `".$trans." Participations`.ParEprCode INNER JOIN `".$trans." Epreuves` ON `".$trans." Epreuves Comp�titions`.EprCompetEprCode = `".$trans." Epreuves`.EprCode INNER JOIN `".$trans." Comp�titions` ON `".$trans." Comp�titions`.CompetCode = `".$trans." Participations`.ParCompetCode INNER JOIN `".$trans." Sports` ON `".$trans." Sports`.SpoCode = `".$trans." Comp�titions`.CompetSpoCode LEFT JOIN `".$trans." Equipes` ON `".$trans." Participations`.ParEquCode = `".$trans." Equipes`.EquCode WHERE `".$trans." Participations`.ParCompetCode = ".$ListeImportCompet);
		if (!(!($reqImport))) {
			while ($resImport = mysql_fetch_array($reqImport)) {
				Maj(1,"Secteurs", $resImport);
				Maj(1,"Etablissements", $resImport, Array("EtabSecCode" => "SELECT SecCode FROM Secteurs WHERE SecLibel = ".$resImport["SecLibel"]));
		...
		$resultLIC = bf_mysql_query("SELECT * FROM Licenci�s");
		$resultPAR = bf_mysql_query("SELECT * FROM Participations");
		$reqImport = bf_mysql_query("SELECT `".$trans." Secteurs`.*, `".$trans." Etablissements`.*, `".$trans." Licenci�s`.*, `".$trans." Participations`.*, `".$trans." Epreuves Comp�titions`.*, `".$trans." Epreuves`.*, `".$trans." Comp�titions`.*, `".$trans." Sports`.* , `".$trans." Equipes`.* FROM `".$trans." Licenci�s` INNER JOIN `".$trans." Etablissements` ON `".$trans." Licenci�s`.LicEtabCode = `".$trans." Etablissements`.EtabCode INNER JOIN `".$trans." Secteurs` ON `".$trans." Etablissements`.EtabSecCode = `".$trans." Secteurs`.SecCode INNER JOIN `".$trans." Participations`On `".$trans." Participations`.ParLicCode = `".$trans." Licenci�s`.LicNumLicence INNER JOIN `".$trans." Epreuves Comp�titions` ON `".$trans." Epreuves Comp�titions`.EprCompetCode = `".$trans." Participations`.ParEprCode INNER JOIN `".$trans." Epreuves` ON `".$trans." Epreuves Comp�titions`.EprCompetEprCode = `".$trans." Epreuves`.EprCode INNER JOIN `".$trans." Comp�titions` ON `".$trans." Comp�titions`.CompetCode = `".$trans." Participations`.ParCompetCode INNER JOIN `".$trans." Sports` ON `".$trans." Sports`.SpoCode = `".$trans." Comp�titions`.CompetSpoCode LEFT JOIN `".$trans." Equipes` ON `".$trans." Participations`.ParEquCode = `".$trans." Equipes`.EquCode WHERE `".$trans." Participations`.ParCompetCode = ".$ListeImportCompet);
		if (!(!($reqImport))) {
			while ($resImport = mysql_fetch_array($reqImport)) {
				Maj(1,"Licenci�s", $resImport, Array("LicEtabCode" => "SELECT EtabCode FROM Etablissements WHERE EtabNum = ".$resImport["EtabNum"]),$resultLIC);
				Maj(1,"Participations", $resImport, Array(
					"ParCompetCode" => "SELECT CompetCode From Comp�titions Where CompetCode = ".$Compet,
					"ParEprCode" => "SELECT EprCompetCode FROM Sports INNER JOIN Epreuves ON Sports.SpoCode = Epreuves.EprSpoCode INNER JOIN `Epreuves Comp�titions` ON `Epreuves Comp�titions`.EprCompetEprCode = Epreuves.EprCode WHERE EprCompetCompetCode = ".$Compet." AND SpoLibelCourt = '".$resImport["SpoLibelCourt"]."' AND EprLibelCourt = '".$resImport["EprLibelCourt"]."'",
			...
			if ($TheFile <> ""){ 
				...
				if (!$ErreurSQL){
					...
					if ($menu == "etablissements") {
						...
						$reqImport = bf_mysql_query("SELECT `".$trans." Secteurs`.*, `".$trans." Etablissements`.*, `".$trans." Licenci�s`.* FROM `".$trans." Licenci�s` INNER JOIN `".$trans." Etablissements` On `".$trans." Licenci�s`.LicEtabCode = `".$trans." Etablissements`.EtabCode INNER JOIN `".$trans." Secteurs` ON `".$trans." Etablissements`.EtabSecCode = `".$trans." Secteurs`.SecCode ".$mastrwhereetab);
						if (!(!($reqImport))) {
							while ($resImport = mysql_fetch_array($reqImport)) {
								Maj(1,"Secteurs", $resImport);
								Maj(1,"Etablissements", $resImport, Array("EtabSecCode" => "SELECT SecCode From Secteurs WHERE SecLibel = ".$resImport["SecLibel"]));
								if (isset($_GET["EtabImport"])) {
									Maj(1,"Licenci�s", $resImport, Array("LicEtabCode" => "SELECT EtabCode FROM Etablissements WHERE EtabNum = ".$resImport["EtabNum"]));
								}
							}
						}
						SupprimeTables($BDD, $trans);
					...
					if ($menu == "licencies") {
						$reqImport = bf_mysql_query("SELECT DISTINCT `".$trans." Secteurs`.*, `".$trans." Etablissements`.* FROM `".$trans." Licenci�s` INNER JOIN `".$trans." Etablissements` On `".$trans." Licenci�s`.LicEtabCode = `".$trans." Etablissements`.EtabCode INNER JOIN `".$trans." Secteurs` ON `".$trans." Etablissements`.EtabSecCode = `".$trans." Secteurs`.SecCode");
						if (!(!($reqImport))) {
							while ($resImport = mysql_fetch_array($reqImport)) {
								Maj(1,"Secteurs", $resImport);
								Maj(1,"Etablissements", $resImport, Array("EtabSecCode" => "SELECT SecCode FROM Secteurs WHERE SecLibel = ".$resImport["SecLibel"]));
						...
						$resultLIC = bf_mysql_query("SELECT * FROM Licenci�s");
						$reqImport = bf_mysql_query("SELECT `".$trans." Secteurs`.*, `".$trans." Etablissements`.*, `".$trans." Licenci�s`.* FROM `".$trans." Licenci�s` INNER JOIN `".$trans." Etablissements` On `".$trans." Licenci�s`.LicEtabCode = `".$trans." Etablissements`.EtabCode INNER JOIN `".$trans." Secteurs` ON `".$trans." Etablissements`.EtabSecCode = `".$trans." Secteurs`.SecCode");
						if (!(!($reqImport))) {
							while ($resImport = mysql_fetch_array($reqImport)) {
								Maj(1,"Licenci�s", $resImport, Array("LicEtabCode" => "SELECT EtabCode FROM Etablissements WHERE EtabNum = ".$resImport["EtabNum"]), $resultLIC);
							}
							bf_mysql_query("INSERT INTO Sports (SELECT `".$trans." Sports`.* FROM `".$trans." Sports` WHERE `".$trans." Sports`.SpoCode = 1)");
							bf_mysql_query("INSERT INTO Cat�gories (SELECT `".$trans." Cat�gories`.* FROM `".$trans." Cat�gories` WHERE `".$trans." Cat�gories`.CatSpoCode = 1)");
						}
						SupprimeTables($BDD, $trans);
						$imp = false;
					}	
					if (($menu == "parametres") && ($sousmenu == "sports") && (!(isset($valideimportsport)))){
						...
						echo "Sport ";
						listederoulante("ListeImportSport", "", "SELECT SpoCode, SpoLibelCourt, SpoLibell� FROM `".$trans." Sports` ORDER BY Ordre ", array("SpoLibelCourt","-","SpoLibell�"), array("","",""), "SpoCode" ,0, 300);
						echo "&nbsp;<input name='valideimportsport' type='submit' id='valideimportsport' value='Importer' class='bouton'>";
						echo "</TD></TR></TABLE></FORM>";
					}	
					
					if (($menu == "competitions") && ($sousmenu == "references") && (!(isset($valideimportcompet)))){
						$reqsport = bf_mysql_query("SELECT SpoLibelCourt FROM Comp�titions INNER JOIN Sports ON Comp�titions.CompetSpoCode = Sports.SpoCode WHERE CompetCode = $Compet");
						if ($reqsport) {
							$ressport = mysql_fetch_array($reqsport);
							if (!(!($ressport))) {
								echo "<FORM method='post'><TABLE CLASS = 'tableconopt'><TR><TD>";
								echo " Importer la comp�tition ";
								listederoulante("ListeImportCompet", "", "SELECT CompetCode, CompetLibell�, DATE_FORMAT(CompetDateD�b,'%d/%m/%Y') AS CompetDateD�b, CompetLieu, SpoLibelCourt FROM `".$trans." Sports` INNER JOIN `".$trans." Comp�titions` ON `".$trans." Sports`.SpoCode = `".$trans." Comp�titions`.CompetSpoCode Where SpoLibelCourt = '".$ressport["SpoLibelCourt"]."'", array("SpoLibelCourt","-","CompetLibell�","-","CompetDateD�b","-","CompetLieu"), array("","","","","","",""),"CompetCode" , 0,  350);
	...
	if (($menu == "outils")  && ($Adm)) {
		...
		if ($tmpName != "") {	
			...
			if ($TheFile<>""){
				...
				MajOrdre(array(array("Cat�gories","CatSpoCode"),array("Epreuves","EprSpoCode")));
				bf_mysql_query("UNLOCK TABLES");
			}
		}
		...
		for ($i = 0; $i < count($mesinfo); $i++) {	
			...
			echo CompteEnr($matable) . " ugselwebv1pt.php" .$manomtable;
				...
				if ( ($CONSULTATION == "Non") && (($_GET["action"] == "exporte") || (isset($exporter)))) {
					...
					if (($optionexporttype == "expser") || ($optionexporttype == "expsqlser") ) {
						...
						if (file_exists("../../inscriptions/$leFic[0]")) {
							$message = "La comp�tition '".$leFic[1]."' a �t� envoy�e.";
							$exp = 0;
						} else {
							$message = "Erreur inatendue. La comp�tition n'a pas �t� envoy�e.";
						}
						VoirMenu();
